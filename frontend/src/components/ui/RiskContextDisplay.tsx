...
const RiskMetric = ({
  ...
  <div className="p-3 bg-background-panel border border-border-subtle rounded-lg hover:border-border-default transition-colors min-w-0">
    <div className="flex items-center gap-2 mb-1.5">
      <Icon className="w-4 h-4 text-text-muted shrink-0" />
      <span className="text-[12px] text-text-muted leading-snug">{label}</span>
    </div>
    <span className={`font-mono text-[15px] font-bold tabular-nums block truncate ${className || 'text-text-primary'}`}>
      {value}
    </span>
  </div>
);
...
export const RiskContextDisplay = ({ riskContext }: RiskContextDisplayProps) => {
  ...
  const drawdownColor =
    currentDrawdown > 0 ? (isDrawdownHigh ? 'text-danger' : 'text-warning') : 'text-success';

  return (
    <div className="card">
      <h3 className="text-[15px] font-semibold text-text-primary mb-4 flex items-center gap-2">
        <ShieldAlert className="w-5 h-5 text-primary-blue shrink-0" />
        Live Risk Context
      </h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <RiskMetric
          ...
          value={`${formatFixed(currentDrawdown)}%`}
          icon={TrendingDown}
        ...
        <RiskMetric label="Capital at Risk" value={formatCurrency(capitalAtRisk)} icon={Wallet} />
        <RiskMetric label="Exposure" value={`${formatFixed(exposurePct)}%`} icon={Gauge} />
        <RiskMetric
          label="Kill Switch"
          value={riskContext.kill_switch_status ? 'ACTIVE' : 'Inactive'}
          icon={Zap}
          className={riskContext.kill_switch_status ? 'text-danger' : 'text-success'}
        />
      </div>
    </div>
  );
